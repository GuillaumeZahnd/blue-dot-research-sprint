...
from balanced_batch_sampler import ExactInterleavedSampler


@dataclass
class TARDataCollator(DataCollatorForSeq2Seq):
    def __call__(self, features: List[Dict[str, Any]], return_tensors=None):
        # Pop all custom fields before parent collator sees the batch
        answer_labels         = [f.pop("answer_labels")         for f in features]
        attack_input_ids      = [f.pop("attack_input_ids")      for f in features]
        attack_attention_mask = [f.pop("attack_attention_mask") for f in features]
        attack_labels         = [f.pop("attack_labels")         for f in features]

        batch = super().__call__(features, return_tensors=return_tensors)

        seq_len        = batch["input_ids"].shape[1]
        attack_seq_len = max(len(x) for x in attack_input_ids)

        # Pad to seq_len (same as input_ids, for answer_labels)
        batch["answer_labels"] = torch.tensor(
            [(al + [-100] * seq_len)[:seq_len] for al in answer_labels],
            dtype=torch.long
        )

        # attack_* sequences may be longer than input_ids (adversarial prompt added) so pad to their own max length independently
        batch["attack_input_ids"] = torch.tensor(
            [(x + [self.tokenizer.pad_token_id] * attack_seq_len)[:attack_seq_len] for x in attack_input_ids],
            dtype=torch.long
        )
        batch["attack_attention_mask"] = torch.tensor(
            [(x + [0] * attack_seq_len)[:attack_seq_len] for x in attack_attention_mask],
            dtype=torch.long
        )
        batch["attack_labels"] = torch.tensor(
            [(x + [-100] * attack_seq_len)[:attack_seq_len] for x in attack_labels],
            dtype=torch.long
        )
...
class TARTrainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        sampler = ExactInterleavedSampler(
